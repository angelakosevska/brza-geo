...
async function scoreRound(round, categories, letter, gameId) {
  ...
  // === ново: земи ги прифатените review зборови за овој круг ===
  ...
  if (gameId) {
    ...
    for (const rw of reviewWords) {
      reviewMap.set(`${String(rw.submittedBy)}_${String(rw.category)}`, rw.word);
    }
  }
  ...
  for (const submission of round.submissions || []) {
    ...
    for (const categoryId of categories) {
      ...
      if (!answer.raw) {
        ...
        // === ново: проверка за review прифатен збор ===
